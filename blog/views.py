...
def get_blog_list_comm_data(request,blogs_all_list):
    paginator = Paginator(blogs_all_list, settings.EACH_PAGE_BLOGS_NUMBER)  # 使用sttings进行分页
    ...
    #获取博客分类的对应博客数量1,转化为sql语句不使用时不占内存
...
def blog_list(request):
    ...
    context = get_blog_list_comm_data(request,blogs_all_list)
...
def blogs_with_date(request,year,month):
    ...
    context = get_blog_list_comm_data(request,blogs_all_list)
    context['blods_with_date'] = '%s年%s月'%(year,month)
